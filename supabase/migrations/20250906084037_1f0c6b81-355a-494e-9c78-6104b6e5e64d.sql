...
SELECT 
    ...
    35,
    '{"contentAnalysis": "Evidence of inappropriate communication", "keyFindings": ["inappropriate language"], "confidenceLevel": 0.75}'::jsonb,
    '{"fileDetails": {"name": "evidence.pdf", "size": 150000, "type": "application/pdf"}}'::jsonb
FROM cases c 
...
-- Add some webhook activity logs with proper JSONB casting
INSERT INTO public.webhook_logs (webhook_type, case_id, payload, response, status, execution_time_ms)
SELECT 
    'CASE_CREATED',
    c.id,
    ('{"event": "case_created", "caseNumber": "' || c.case_number || '"}')::jsonb,
